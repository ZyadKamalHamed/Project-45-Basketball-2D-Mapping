...
class ObjectTracking:
    ...
    def __init__(
        ...
        output_path="object-tracking.mp4",
        ...
        # Video writing module (initialized lazily after first frame read)
        self.output_path = output_path
        self.output_fps_fallback = float(output_fps_fallback)
        self.writer = None
    ...
    def _init_writer_from_frame(self, frame):
        h, w = frame.shape[:2]
        fps = float(self.cap.get(cv2.CAP_PROP_FPS) or 0.0)
        if not np.isfinite(fps) or fps <= 1e-3:
            fps = self.output_fps_fallback

        # Try a few codecs for best compatibility depending on local OpenCV build.
        # Primary target: MP4 container.
        ...
        raise RuntimeError(
            ...
            f"Tried codecs: {fourcc_candidates}. "
            f"Computed fps={fps}, size={(w, h)}."
        )
    ...
    def run(self):
        ...
        while self.cap.isOpened():
            ...
            if self.writer is None:
                # Ensure we're writing a standard 3-channel BGR frame.
                if len(im0.shape) == 2 or (len(im0.shape) == 3 and im0.shape[2] == 1):
                    im0 = cv2.cvtColor(im0, cv2.COLOR_GRAY2BGR)
                self._init_writer_from_frame(im0)
