...
def _assign_likely_shooter(
    ...
    """Pick the player who most likely shot. Aggregated over a window — not single-frame.

    Why aggregate, not per-frame: at the exact release moment the shooter has extended
    the ball upward away from their body, so a contesting defender's torso can briefly be
    closer to the ball than the shooter's. The shooter is the player who was closest to
    the ball *while holding it*, which means closest over many frames, not just at one
    instant. So we group observations by tracker_id and score each track by its combined
    closeness (min distance + a small mean term).
    """
    release_frame = int(candidate["release_frame"])

    # Collect per-track observations across the lookback window.
    ...
    # Require each candidate to appear in at least a third of the lookback window. This
    # filters brief defender intrusions (someone who crossed in front of the ball for
    ...
    for tid, observations in per_track.items():
        if len(observations) < min_observations:
            continue
        distances = [o["distance"] for o in observations]
        min_dist = min(distances)
        mean_dist = sum(distances) / len(distances)
        # Combined score — heavily weights the minimum distance (the player *did* get
        # near the ball) but adds a small mean term (the player *stayed* near it).
        score = min_dist + 0.3 * mean_dist
        if best is None or score < best["score"]:
            # For the bbox we report, pick the observation closest to the release frame.
            # That's what gets used for the team re-classification at release.
            closest_to_release = min(
                observations, key=lambda o: abs(o["frame"] - release_frame)
            )
            best = {
                "score": score,
                "frame": closest_to_release["frame"],
                "track_id": tid,
                "distance_px": min_dist,
                "bbox": closest_to_release["bbox"],
            }

    # Fallback: if no track met the observation threshold (short clips, occlusion,
    # tracker re-spawning IDs), fall back to the original per-(frame, player) pick so
    # the shot still gets a shooter attribution rather than being silently dropped.
    if best is None:
        for tid, observations in per_track.items():
            for obs in observations:
                dist = obs["distance"]
                temporal_penalty = (release_frame - obs["frame"]) * 2.0
    ...
    return {
        "shooter_track_id": int(best["track_id"]),
